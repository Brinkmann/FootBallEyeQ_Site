...
function CatalogPageContent({ initialExercises }: CatalogPageClientProps) {
  const [exercises, setExercises] = useState<Exercise[]>(initialExercises);
  const [searchQuery, setSearchQuery] = useState("");
  const [showFavoritesOnly, setShowFavoritesOnly] = useState(false);
  const [initialFavorites, setInitialFavorites] = useState<Set<string> | null>(null);
  const [isLoading, setIsLoading] = useState(initialExercises.length === 0);
  ...
  const fetchExercises = useCallback(async (options: { withSpinner?: boolean } = {}): Promise<void> => {
  ...
  useEffect(() => {
    if (initialExercises.length > 0) {
      setIsLoading(false);
      fetchExercises({ withSpinner: false });
    } else {
      fetchExercises({ withSpinner: true });
    }
  ...
  }, [initialExercises.length, fetchExercises]);
